...
function transformCustomer(value)
{
    ...
    if (!customer)
        customer = site.customers.newCustomer(value);
...
function transformProduct(value)
{
    ...
    if (!productFamily)
        productFamily = site.productFamilies.newProductFamily(value.product_family.name, value.product_family.id, value.product_family.handle);

    var product = productFamily.products.find({ id: value.id });
    if (!product)
        product = productFamily.products.newProduct(_.omit(value, 'product_family'));
...
var Subscription = Entity.extend(SubscriptionSchema, {
    ...
    postCharge: function(amount, memo, useNegativeBalance, delayCapture)
    {
        ...
        return charge.save()
            .then(function()
            {
                console.log(charge.toObject({ all: true }));
            })
    },
                ...
                if (!sub.payment_profile_id)
                {
                    ...
                    if (creditCardChanges.length)
                        ...
                        if (self.creditCard.id)
                            sub.payment_profile_id = self.credit_card.id;
                }
