...
var SubscriptionSchema = new Entity.SchemaObject({
    ...
    product_handle: { type: String,toObject:'hasValue' },
    product_id: { type: Number,toObject:'hasValue' },
    customer_id: { type: Number,toObject:'hasValue'},
    customer_reference: { type: String,toObject:'hasValue' },
    payment_profile_id: { type: String,toObject:'hasValue' },
    ...
    cancellation_message: { type: String, toObject: 'hasValue' },
    next_billing_at: { type: Date },
    vat_number: { type: String, toObject: 'hasValue' },
    coupon_code: { type: String, toObject: 'hasValue'},
    payment_collection_method: { type: String },
    expiration_tracks_next_billing_change: { type: Boolean },
    agreement_terms: { type: Boolean },
    product_change_delayed: { type: Boolean, toObject: 'hasValue' },
    product_price_in_cents: { type: Number, toObject: 'never' },
    product_version_number: { type: Number, toObject: 'never' },
...
var Subscription = Entity.extend(SubscriptionSchema, {
    constructor: function (data, options)
    {
        ...
        var customer,components,product,creditCard;
        if (dataCopy)
        {
            if (dataCopy.customer)
            {
                customer = _.extend({},dataCopy.customer);
                delete dataCopy.customer;
            }
            if (dataCopy.credit_card || dataCopy.credit_card_attributes)
            {
                creditCard = _.extend({},dataCopy.credit_card, dataCopy.credit_card_attributes);
                delete dataCopy.credit_card;
                delete dataCopy.credit_card_attributes;
            }

            if (dataCopy.product)
            {
                product = _.extend({},dataCopy.product);
                delete dataCopy.product;
            }

        }

        this.Entity.call(this, dataCopy, options);
        if (customer)
            transformCustomer.call(this,customer);
        if (product)
            transformProduct.call(this,product);
        if (creditCard)
            transformCreditCard.call(this,creditCard);
    ...
    adjustment: function(amount, memo, method)
    {
        var adjustment = new Adjustment({ amount: amount, memo: memo, adjustment_method: method}, { parent: this });
    ...
    save: function(options)
    {
        ...
        return Entity.Promise()
            ...
            .then(function ()
            {
                var o = { subscription: _.omit(self.toObject({ hasValue: true }), 'productId','couponCode','_customer','_components','_credit_card','_product','product', 'customer', 'credit_card_attributes') };
                var sub = o.subscription;
                if (!sub.payment_profile_id && !self.id)
                {
                    if (self.creditCard || self.creditCardAttributes)
                    {
                        var cc = self.creditCard || self.creditCardAttributes;
                ...
                if (!sub.product_handle || !sub.product_id && !self.id)
                {
                    if (self.product)
                    {
                ...
                if (sub.product_id )
                {
                    delete sub.product_handle;
                }

                if (!self.id)
                    if (!sub.customer_id && !sub.customer_reference && self.customer)
                    {
