...
_.extend(Collection.prototype, {
    ...
    pagedLoad: function ()
    {
        ...
        function loadPage(p, pageNumber)
        {
            ...
            var o = { page: pageNumber };
            if (options.perPage > 0)
                o.per_page = options.perPage;
            if (qs.stringify(o).length)
                var currentPath = options.path + (options.path.indexOf('?') >= 0 ? '&' : '?') + qs.stringify(o);
            return self.parentSite.request('get', currentPath)
                .then(function ()
                {
                    var args = arguments;
                    var q = Collection.Promise.denodeify(function (callback)
                    {
                        var response = args[0][0];
                        var body = args[0][1];
