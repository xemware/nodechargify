...
_.extend(RequestWrapper.prototype,
    {
        ...
        {
            var url,auth;
            options = _.defaults({}, options, { retryCount: 1});
            if (options.v2)
            {
            ...
            function doRequest()
            {
                ...
                {
                    if (err)
                        logger.debug(util.format('Error - %s - %s - %s', response && response.statusCode || '(No response)', method, err));
                    else
                        logger.debug(util.format('Response %d - %s - %s', response && response.statusCode || '(No response)', method,url));
        ...
        del: function (path, options, callback)
        {
            this.req('del', path, options, callback);
        }
